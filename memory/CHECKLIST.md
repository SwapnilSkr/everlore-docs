...
- [x] Choice POV drift: tap-to-play choices conflate viewpoints (e.g. "The
      Unseen Child" — label "Observe the son" with send "*I glance at my
      brother*", though the protagonist IS the son).
      (Server commits `dbc1cf5` + `ac5b54e`. Root cause: choices are generated by the
      context-blind `extractSceneMetadata` pass — the narrator is always
      `proseOnly`, so this cheap second model is the SOLE source of choices, yet
      it only received a generic perspective line and never the protagonist's
      identity. In third-person GM prose it could not resolve which character is
      the player, so "I" drifted. Fix threads the protagonist identity (name +
      merged role-aliases + persona) from the generation processor into the
      extractor and requires BOTH label and send to be written strictly
      first-person from the protagonist; sentient worlds anchor to the persona
      name and frame choices as the player's own moves. Stale "NSFW path only"
      docstring refreshed. `ac5b54e` dropped a dead `persona` text field (threaded
      but never read — only name+aliases anchor the POV; personality reaches
